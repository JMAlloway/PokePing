"""Ace Hardware retailer monitor.

Ace Hardware occasionally sells Pokemon TCG products online.
...
def extract_product_id(url: str) -> str | None:
    """Extract Ace Hardware product ID from URL.

    Handles:
      - https://www.acehardware.com/departments/toys/product-name/1234567
      - 1234567
    """
    match = re.search(r"/(\d{7,})$", url.rstrip("/"))
    if match:
        return match.group(1)
    match = re.search(r"^(\d{7,})$", url.strip())
    if match:
        return match.group(1)
    return None


class AceHardwareMonitor(RetailerMonitor):
    name = "acehardware"
    ...
    async def check_scrape(self, product_url: str, product_name: str) -> ProductResult:
        ...
        if json_ld:
            try:
                ...
                logger.debug("Failed to parse Ace Hardware JSON-LD: %s", exc)
