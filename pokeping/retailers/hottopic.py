"""Hot Topic retailer monitor.

Hot Topic sells Pokemon TCG products and exclusives online.
...
def extract_product_id(url: str) -> str | None:
    """Extract Hot Topic product ID from URL.

    Handles:
      - https://www.hottopic.com/product/pokemon-tcg-name/12345678.html
      - 12345678
    """
...
class HotTopicMonitor(RetailerMonitor):
    name = "hottopic"
    base_url = "https://www.hottopic.com"
    ...
    async def check_scrape(self, product_url: str, product_name: str) -> ProductResult:
        ...
        if json_ld:
            try:
                ...
                logger.debug("Failed to parse Hot Topic JSON-LD: %s", exc)
        ...
        if price_float is None:
            price_el = soup.find("span", {"class": re.compile(r"price-sales|product-price", re.I)})
            if price_el:
        ...
        if not image_url:
            img = soup.find("img", {"class": re.compile(r"product-image|primary-image", re.I)})
            if img:
    ...
    def build_atc_url(self, product_url: str) -> str | None:
        ...
        if not product_id:
            return None
        return f"https://www.hottopic.com/on/demandware.store/Sites-hottopic-Site/default/Cart-AddProduct?pid={product_id}&quantity=1"
