"""Dick's Sporting Goods retailer monitor.

Dick's Sporting Goods sells Pokemon TCG products online at MSRP.
...
def extract_product_id(url: str) -> str | None:
    """Extract Dick's product ID from URL.

    Handles:
      - https://www.dickssportinggoods.com/p/product-name-12345_12345-12345
      - https://www.dickssportinggoods.com/p/product-name/12345678
    """
    match = re.search(r"/p/[^/]+/(\w+)$", url.rstrip("/"))
    if match:
        return match.group(1)
    match = re.search(r"/p/([^/]+)$", url.rstrip("/"))
    if match:
        return match.group(1)
    return None


class DicksMonitor(RetailerMonitor):
    name = "dicks"
    ...
    async def check_scrape(self, product_url: str, product_name: str) -> ProductResult:
        ...
        if json_ld:
            try:
                ...
                logger.debug("Failed to parse Dick's JSON-LD: %s", exc)
